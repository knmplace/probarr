...
# Ordered to match the actual workflow: set up a provider, curate the
# channels you want, then Runs is where results and history live.
_NAV_ITEMS = [("providers", "/providers", "Providers"),
...
def topbar(label="", active="", right="", home=True):
    ...
    if home:
        ...
        nav = '<div class="nav">' + "".join(
            ...
            f'{text}</button></a>' for key, href, text in _NAV_ITEMS) + '</div>'
