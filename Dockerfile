# probarr - IPTV stream verification and visual curation
#
...
# Python dependencies at all -- probarr is standard library only.
FROM alpine:3.20

RUN apk add --no-cache python3 ffmpeg tzdata ca-certificates

WORKDIR /app
COPY probarr/ /app/probarr/

ENV PYTHONUNBUFFERED=1 \
    PYTHONDONTWRITEBYTECODE=1 \
    PROBARR_CONFIG=/config \
    PROBARR_PORT=7799
